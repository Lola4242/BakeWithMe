...
public class RecipeProvider extends ContentProvider {
    ...
    public static final int CODE_STEP = 101;
     ...
     * signifies that this UriMatcher is a static member variable of RecipeProvider and is a
    ...
    public static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(authority, RecipeContract.PATH_STEP, CODE_STEP);
        matcher.addURI(authority, RecipeContract.PATH_INGREDIENT, CODE_INGREDIENT);
    ...
    public boolean onCreate() {
         ...
         * lengthy operations will cause lag in your app. Since RecipeDbHelper's constructor is
    ...
    public int bulkInsert(@NonNull Uri uri, @NonNull ContentValues[] values) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case CODE_STEP:
                db.beginTransaction();
                int rowsInsertedStep = 0;
                try {
                    for (ContentValues value : values) {
                        long _id = db.insert(RecipeContract.StepEntry.TABLE_NAME_STEP, null, value);
                        if (_id != -1) {
                            rowsInsertedStep++;
                        }
                    }
                ...
                if (rowsInsertedStep > 0) {
                    getContext().getContentResolver().notifyChange(uri, null);
                }

                return rowsInsertedStep;
            case CODE_INGREDIENT:
                db.beginTransaction();
                int rowsInsertedIngredient = 0;
                try {
                    for (ContentValues value : values) {
                        long _id = db.insert(RecipeContract.IngredientEntry.TABLE_NAME, null, value);
                        if (_id != -1) {
                            rowsInsertedIngredient++;
                        }
                    }
                    db.setTransactionSuccessful();
                } finally {
                ...
                if (rowsInsertedIngredient > 0) {
                    getContext().getContentResolver().notifyChange(uri, null);
                }

                return rowsInsertedIngredient;
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case CODE_STEP: {
                cursor = mOpenHelper.getReadableDatabase().query(
                        RecipeContract.StepEntry.TABLE_NAME_STEP,
                        projection,
                        selection,
                        selectionArgs,
                        null,
     ...
     * override it as RecipeProvider extends ContentProvider and getType is an abstract method in
    ...
    /**
     * In Sunshine, we aren't going to do anything with this method. However, we are required to
     * override it as RecipeProvider extends ContentProvider and insert is an abstract method in
     * ContentProvider. Rather than the single insert method, we are only going to implement
     * {@link RecipeProvider#bulkInsert}.
     *
